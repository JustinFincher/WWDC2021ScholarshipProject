        ...
        ScrollView(.vertical, showsIndicators: true, content: {
            VStack(alignment: .leading, spacing: 8, content: {
                ...
                Text("Ask the person you wish to scan to maintain the T-pose or 大-pose. The pose should be maintained when you are reading the skeleton position.")
                    .font(.caption)
                Text("Point your iPad torwards to the person you wish to scan. You should be able to see a skeleton highlighting the joints and bones. Move around a little bit to align the skeleton witht the human body, then stand still, and tap next to fix any positioning issues.")
                    .font(.caption)
                
                Button(action: {
                    OperationManager.shared.goTo(mode: .positionSekeleton) { }
                }, label: {
